...
const db = mongoose.connect('mongodb://thuongthanhto:toilaThuong1@ds129004.mlab.com:29004/thuongthanhto');
...
function getTimeStamp(res, callback) {
    TimeStamp.find({
    }, function (err, data) {
        if (err) {
            callback('error');
...
function returnObjectLender(lenderName, listLender, listValue, fix30, fix15, arm51, arm71, arm101) {
    var list30 = [];
    var list15 = [];
    var list51 = [];
    var list71 = [];
    ...
    if (fix30 == '' && fix15 == '') {
        ...
        var myJSONObject = {
            lender_name: lenderName,
            RATE_JUMBO: { "30-FIXED": list30, "15-FIXED": list15, "5/1-ARM": list51, "7/1-ARM": list71, "10/1-ARM": list101 },
        }
    }
...
function getRawDataOfAllLenderById(res, cookie, id, callback) {
    request({
        method: 'GET', url: 'https://epps.elliemae.com/pricing/qsub.aspx?Fn=qualify&undefined', headers: {
